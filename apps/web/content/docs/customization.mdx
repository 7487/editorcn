...
Replace any built-in icon via the `icons` prop. All 27 icons accept a React node — use inline SVGs, emoji, or any icon library.
...
Override per-language icons in the code block language selector via the `languageIcons` key:
